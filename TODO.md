...
- [x] ~~`[F-03]` 配置Git提交信息规范检查与预提交钩子（使用commitlint & husky）~~ (`复杂度: S, 估算: 1 PD`) ❌ **已废弃** *2025-10-25 项目不需要该需求*
- [x] `[F-04]` 配置CI/CD流水线（如GitHub Actions），实现版本发布（release）和Docker镜像自动构建推送 (`复杂度: M, 估算: 3 PD`) ✅ **已完成** *2025-10-28*
...
### ① 后端 (Backend - 核心引擎)
- [x] `[B-04]` TMDB API 客户端封装：实现根据剧集信息查询中文别名，包含缓存和错误处理机制 (`复杂度: M, 估算: 3 PD`)
- [x] `[B-05]` Prowlarr API 客户端封装：实现向Prowlarr发起搜索请求 (`复杂度: S, 估算: 2 PD`) ✅ **已完成** *2025-12-02 完成搜索功能与REST端点实现*
- [x] `[B-06]` **[核心]** 标题解析器 V1：开发核心解析逻辑，支持至少5种常见的国漫资源命名格式，能准确提取集号、质量等信息 (`复杂度: XL, 估算: 8 PD`) ✅ **已完成** *2026-01-17*
- [x] `[B-07]` **[核心]** Torznab XML 生成模块：根据解析结果，生成符合Sonarr规范的XML响应 (`复杂度: L, 估算: 5 PD`) ✅ **已完成** *2026-01-24*
- [ ] `[B-08]` **[核心]** 端到端自动化引擎编排：实现Torznab API端点，将`[B-04]`至`[B-07]`的模块串联起来，完成完整的请求处理-增强-搜索-解析-响应流程 (`复杂度: L, 估算: 4 PD`)
- [ ] `[B-12]` 后端 API 响应模型统一重构：在完成 `[B-04]` ~ `[B-08]` 并通过初步验收后，设计并引入泛型响应模型（如 `ApiResponse[T]`），替换现有基于 `success_response` / `error_response` 的手工字典构造，使所有后端端点的入参/出参 schema 与 OpenAPI 更加一致 (`复杂度: M, 估算: 3 PD`)
