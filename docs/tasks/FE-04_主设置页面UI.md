...
### 4.1 代理配置卡片（单行横向）
- 字段：
  - `address` 代理地址（含协议，支持 `http/https/socks5`，例如 `http://127.0.0.1:7890`）
  - `testUrl` 测试地址（可选，留空使用后端默认，例如 `https://www.google.com/generate_204`）
  - `timeout` 超时时间（毫秒，范围 1000-30000）
- 布局：大屏三列并排（地址、测试地址、超时时间）
  - 列宽建议：`address` 与 `testUrl` 等宽（各占约 5/12），`timeout` 略窄（占约 2/12）
  - 中小屏响应式改为单列纵向堆叠
  - 底部提供说明提示区
- 操作：重置、测试连通性、保存代理
- 校验：
  - `address` 必须为合法 URL 且包含协议（协议即类型）
  - `testUrl` 若填写则需为合法 URL
  - 地址为空时禁用测试按钮
...
- 文案：在代理卡片说明“代理类型通过地址中的协议自动识别（无需单独选择）”。
- 布局一致性：保证 `address` 与 `testUrl` 输入宽度一致；`timeout` 输入适当缩短但与其余字段在同一行垂直居中对齐。
