...
class TMDBClient(ExternalServiceClient):
    ...
    async def search_tv_async(
        ...
        return await self._get_async("/search/tv", params=params)
    ...
    async def get_alternative_titles_async(
        self,
        tv_id: int,
        ...
        if effective_country:
            if isinstance(effective_country, str) and len(effective_country) == 2 and effective_country.isalpha():
                ...
                return False, "参数错误: country 必须为2位字母的国家码"
        return await self._get_async(f"/tv/{tv_id}/alternative_titles", params=params)
    ...
    async def get_tv_details_async(self, tv_id: int, language: Optional[str] = None) -> tuple[bool, Any]:
        """
        获取电视剧详情（异步）
        """
        if not isinstance(tv_id, int) or tv_id <= 0:
        ...
        return await self._get_async(f"/tv/{tv_id}", params=params)
    ...
    async def find_by_external_id_async(
        self,
        external_id: str,
        external_source: str,
    ) -> tuple[bool, Any]:
        """
        通过外部 ID 查找 TMDB 数据（异步）

        Args:
            external_id: 外部 ID
            external_source: 外部来源 (imdb_id/tvdb_id/tvrage_id)
        """
        if not external_id or not str(external_id).strip():
            return False, "参数错误: external_id 不能为空"
        if external_source not in {"imdb_id", "tvdb_id", "tvrage_id"}:
            return False, "参数错误: external_source 非法"

        params = self._add_api_key_to_params({"external_source": external_source})
        return await self._get_async(f"/find/{external_id}", params=params)
